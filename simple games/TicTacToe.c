...
int main()
{
    ...
    while (time != 10)
    {
        ...
        scanf("%d", &input);
        if (pos[(input - 1) / 3][(input - 1) % 3] == true || input > 9 || input < 1)
        {
            printf("input again !\n");
            continue;
        }
        ...
        if ((table[0][0] == OX[time % 2] && table[1][1] == OX[time % 2] && table[2][2] == OX[time % 2]) ||
            (table[2][0] == OX[time % 2] && table[1][1] == OX[time % 2] && table[0][2] == OX[time % 2]))
        {
            printf("%c player win !\n", (time % 2) ? 'A' : 'B');
            system("pause");
            return 0;
